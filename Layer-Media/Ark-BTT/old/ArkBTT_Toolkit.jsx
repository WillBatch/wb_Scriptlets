// Create a dockable UI with two buttons
function createUI() {
  var scriptName = "Ark BTT Toolkit";
  var dockablePanel = new Window("palette", scriptName, undefined, {
    resizeable: true,
  });
  dockablePanel.orientation = "column";
  var group1 = dockablePanel.add("group"); // Create a group for the first row
  group1.orientation = "row"; // Set the group orientation to "row"
  var group2 = dockablePanel.add("group"); // Create a group for the first row
  ...
  var group3 = dockablePanel.add("group"); // Create a group for the first row
  ...
  dockablePanel.onResizing = dockablePanel.onResize = function () {
    this.layout.resize();
  };

  dockablePanel instanceof Window
    ? (dockablePanel.show(), (dockablePanel = dockablePanel))
    : (dockablePanel.layout.layout(true), dockablePanel.layout.resize());
}

// Run the createUI function
createUI();
...
function setupForm(formcompstring) {
  ...
  if (comp.selectedLayers.length == 1) {
    ...
    if (foundComposition) {
      copyPasteFormToActiveComp(foundComposition, comp, selLayer);
    } else {
    ...
    function copyPasteFormToActiveComp(formComp, currentComp, fill_layer) {
      ...
      currentComp.layer(1).source.width = comp.width;
      currentComp.layer(1).source.height = comp.height;
      currentComp
        .layer(1)
        .property("Transform")
        .property("position")
