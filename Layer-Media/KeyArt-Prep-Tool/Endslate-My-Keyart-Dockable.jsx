...
var myPanel = new Window("palette", "Endslate My", undefined, {
  resizeable: true,
});

// Create a group to hold UI elements
var myGroup = myPanel.add("group");
myGroup.orientation = "column";

// First row: "Endslate My Keyart" button
var endslateButton = myGroup.add("button", undefined, "Endslate My Keyart");
endslateButton.onClick = function () {
  // Add your logic for "Endslate My Keyart" button here
  executeMainScript();
};

// Second row: Number input field and "Update Z Index" button
var secondRowGroup = myGroup.add("group");
var numberInput = secondRowGroup.add("edittext", undefined, "");
numberInput.characters = 3; // Set a smaller number of characters for the text field
numberInput.text = "0"; // Set default value to 0

var updateZIndexButton = secondRowGroup.add(
  "button",
  undefined,
  "Update Z Index"
);
updateZIndexButton.onClick = function () {
  var zIndexValue = parseInt(numberInput.text, 10);
  if (!isNaN(zIndexValue)) {
    // Add your logic for "Update Z Index" button here
    executeSecondScript(zIndexValue);
  } else {
...
function executeMainScript() {
  ...
  var layerCompTemplateFolder = checkForProjectItem(
    4,
    "02_Layer Comp Template"
  );
  var layerCompTemplate = checkForProjectItem(
    5,
    "TemplateComp-DO NOT MODIFY_1"
  );
  ...
  if (
    ...
    runScript();
    app.endUndoGroup();
    alert("Your Keyart has been Endslated. Time to add the juice.");
  }
    ...
    if (psdLayerComp === undefined || zeroLayer === undefined) {
      ...
      for (var i = 0; i < template_comps_array.length - 1; i++) {
        ...
        for (var b = 1; b < app.project.activeItem.numLayers; b++) {
          if (
            app.project.activeItem.layer(b).name ===
            template_comps_array[i].name
          ) {
      ...
      app.project.activeItem.layer(1).enabled = false;
...
function executeSecondScript(textInput) {
  var markerValue = new MarkerValue(textInput);

  for (var i = 0; i < app.project.activeItem.selectedLayers.length; i++) {
    app.project.activeItem.selectedLayers[i].marker.setValueAtTime(
      0,
      markerValue
    );
  }
